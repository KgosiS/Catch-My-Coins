...
class IncomeAnalysisFragment : Fragment(R.layout.fragment_income_analysis) {
    ...
    private fun fetchIncomeTransactions() {
        ...
        if (start.isNotEmpty() && end.isNotEmpty()) {
            val transactionList = dbHelper.getIncomeTransactionsByDateRange(start, end)
            val transactions = dbHelper.getIncomeTransactionsByDateRange(start, end)
    ...
    private fun setupPieChart(expenses: List<Expense>) {
        ...
        entries.add(PieEntry(totalExpenses.toFloat(), "Total Income"))
    ...
    private fun displayCategoryTotals(expenses: List<Expense>) {
        ...
        if (expenses.isEmpty()) {
            val noDataView = TextView(requireContext()).apply {
                text = "No income found for this range."
